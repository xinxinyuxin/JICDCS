...
class Trainer:
    ...
    def __init__(self,model,train_dl,val_dl,test_dl,loss,optim,lr,metrics,device,reset_fn=[],model_name='model',verbose=2):
        """Initialize the trainer instance##初始化训练器实例
        - **model** -- progress bar object to update  模型对象
        - **train_dl** -- training dataloader    训练数据集的数据加载器
        - **val_dl** -- validation dataloader  验证数据集的数据加载器
        - **test_dl** -- test dataloader  测试数据集的数据加载器
        - **loss** -- loss function, must return a PyTorch tensor, not scalar  a损失函数，必须返回一个PyTorch张量而不是标量
        - **opt_name** -- name of the optimizer. Either of  ['adamfast','cgd','adam','vanilla_adam']  优化器的名称，可选值为 ['adamfast', 'cgd', 'adam', 'vanilla_adam']
        - **lr** -- initial learning rate    初始学习率
        - **metrics** -- Python dictionary with format 'metric_name':(func,max/min), where func is any function with inputs target,output and should return a scalar value for accuracy. max/min defines the desired optimization of this metric  一个Python字典，格式为'metric_name': (func, max/min)，
        ##其中func是任何具有目标值和输出值输入的函数，并且应该返回一个标量值以计算准确性。max/min定义该指标的优化方向。
        - **device** -- device on which to train, either of ['cpu','cuda:x'] where x is the index of gpu. Multi-GPU training is not supported yet.  训练所使用的设备，可选值为 ['cpu', 'cuda:x']，其中x是GPU的索引。暂不支持多GPU训练。
        - **reset_fn** -- function to reset network parameters. Called at the start of each run  重置网络参数的函数。在每次运行开始时调用。
        - **track** -- metric to track in format ['mode','metric_name','max/min']
        - **model_name** -- filename of the saved model 保存模型的文件名
        - **verbose** -- extent of debug output: 0=No output, 1=show only run progress bar, 2=show run and epoch progress bar
        ##调试输出的程度：0=无输出，1=仅显示运行进度条，2=显示运行和轮次的进度条
        """
    ...
    def init_stats(self,num_epochs,num_runs):
        """Initialize containers for storing statistics and models初始化用于存储统计信息和模型的容器
        ...
        num_epochs -- number of epochs per run  每个运行中的轮次数
        num_runs -- number of randomly initialized runs  随机初始化的运行次数
        这段代码的作用是根据指定的轮次数（num_epochs）和运行次数（num_runs）初始化用于存储统计信息的容器。
        """
    ...
    def _log_to_pbar(self,pbar,idx,modes):
        """Calculate and show statistics to show in progress bar 计算并在进度条中显示统计信息
        - pbar -- progress bar object to update  更新进度条的对象
        - idx -- a list [r,e] corresponding to run, and epoch index. e and b are optional.  一个包含运行和轮次索引的列表 [r, e]。e和b是可选的。
        - modes -- some of ['train','val','test']. Default: train  一些模式的列表，包括 ['train', 'val', 'test']。默认值为 train
        """
        log = {}
        for mode in modes:
            if len(self.dl[mode])==0: continue
