...
class OpTier(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,operators,OPLIB,padding=-1,sampling_factor=1,layer_idx=-1,optimize=True):
        ...
        self.operators = operators##确保 operators 列表的长度与输出通道数目一致。
        ...
        if optimize:
            ...
            for op_idx_now in operators:
                self.oper.append(OpBlock(in_channels,1,kernel_size,op_idx_now,OPLIB))##op_idx_now 表示该操作的索引。
    ...
    def forward(self,x):
        self.output = []
        # Forward Propagation
        if self.padding==-1: padding = int(np.ceil(self.kernel_size/2))-1##确保卷积操作在处理输入数据时保持输入和输出的形状尺寸一致。
